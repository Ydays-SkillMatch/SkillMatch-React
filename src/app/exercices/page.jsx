...
export default function ExercicesList() {
      ...
      try {
        ...
        if (data && data.exercices) {
          ...
          const exercisesWithDates = data.exercices.map((ex, index) => ({
            ...ex,
            dueDate: new Date(
              now.getTime() + (index + 1) * 3 * 24 * 60 * 60 * 1000,
            ).toISOString(),
            organization: "SkillMatch Academy",
            language: ex.language || defaultLanguage, // Assurer que chaque exercice a un langage
          }));

          const sortedExercises = exercisesWithDates.sort(
  ...
  const handleLogout = () => {
    document.cookie =
      "SkillMatchToken=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
    router.push("/");
  };
  ...
  // Fonction pour obtenir le nom complet du langage
  ...
  // Fonction pour obtenir la couleur du badge du langage
  const getLanguageColor = (langCode) => {
    const colors = {
      js: "bg-yellow-800 text-yellow-200",
      ts: "bg-blue-800 text-blue-200",
      py: "bg-blue-900 text-blue-200",
      java: "bg-orange-900 text-orange-200",
      c: "bg-gray-800 text-gray-200",
      cpp: "bg-indigo-900 text-indigo-200",
      cs: "bg-purple-900 text-purple-200",
      php: "bg-purple-800 text-purple-200",
      rb: "bg-red-900 text-red-200",
      go: "bg-cyan-900 text-cyan-200",
      rs: "bg-orange-800 text-orange-200",
      swift: "bg-orange-700 text-orange-200",
      kt: "bg-purple-800 text-purple-200",
      html: "bg-orange-800 text-orange-200",
      css: "bg-blue-800 text-blue-200",
    };

    return colors[langCode] || "bg-gray-700 text-gray-200";
  };
          ...
          {exercises.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center">
              {exercises.map((exercise) => {
                ...
                const languageLabel = getLanguageLabel(exercise.language);
                const languageColor = getLanguageColor(exercise.language);
                    ...
                    <div className="p-5">
                      <div className="flex justify-between items-center mb-2">
                        <div className="text-sm text-gray-400">
                          {exercise.organization || "SkillMatch"}
                        </div>
                        <span
